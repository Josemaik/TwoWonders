...
void AttackSystem::createAttackRangedOrMelee(EntityManager& em, Entity& ent, AttackComponent& att, bool isRanged) {
    ...
    auto& r = em.addComponent<RenderComponent>(e, RenderComponent{ .position = phy.position + (isRanged ? vec3d{0, 0, 0} : att.vel * 2), .scale = { isRanged ? 0.5 : 1.0, isRanged ? 0.5 : 1.0, isRanged ? 0.5 : 1.0 }, .color = BLACK });

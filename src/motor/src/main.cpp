...
DarkMoon::Node* createHUD(DarkMoon::DarkMoonEngine& engine){
    ...
    // Node: TextBox
    engine.CreateTextBox({660, 20}, {120, 60}, D_AQUA_LIGHT, "Texto", engine.GetDefaultFont(), 24, D_BLACK, DarkMoon::Aligned::CENTER, DarkMoon::Aligned::CENTER, "Caja de texto de prueba", p_nodeHUD);

    // Node: Button
    auto p_nodeButton = engine.CreateButton({660, 100}, {120, 60}, "Boton", engine.GetDefaultFont(), 24, D_BLACK, DarkMoon::Aligned::CENTER, DarkMoon::Aligned::CENTER, D_ORANGE_DARK, D_ORANGE, D_ORANGE_LIGHT, "Boton de prueba naranja", p_nodeHUD);
...
DarkMoon::Node* createMainCharacter(DarkMoon::DarkMoonEngine& engine){
    // Node: Modelo
    auto model = engine.CreateModel("assets/Nomada.obj", D_WHITE, "Modelo: Main Character", engine.GetRootNode());
...
int main(){
    ...
    if(engine.InitWindow(800, 600, "DarkMoon Engine")){
        ...
        auto button = createHUD(engine);

        auto buttonEntity = dynamic_cast<DarkMoon::Button*>(button->getEntity());

...
int main() {
    ...
    if (engine.InitWindow(800, 600, "DarkMoon Engine")) {
        ...
        auto varitaModel = engine.CreateModel("assets/MainCharacter/Varita.obj", D_WHITE, "Modelo: Varita", engine.GetRootNode());
        varitaModel->setTranslation({ 0.0f, 05.0f, 05.0f });
        auto eVaritaModel = dynamic_cast<DarkMoon::Model*>(varitaModel->getEntity());
        eVaritaModel->drawModel = true;

        // auto& characterMeshes = modelcharacter->getAdjuntMeshes();
        // for (auto& mesh : eVaritaModel->getMeshes())
        // {
        ...
        auto id = am.PlayAnimation(anims[0]);
        for (auto& mesh : m_meshes) {
            if (mesh->hasBones)
                mesh->animID = id;
        }
        ...
        while (!engine.WindowShouldClose()) {
            ...
            varitaModel->setTransformationMatrix(combinedTransform);
            // for (int i = 0; i < 4; i++)
            // {
            //     for (int j = 0; j < 4; j++)
            //     {
